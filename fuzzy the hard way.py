...
speed_level_right = fuzz.interp_membership(x_speed, speed_right,
                                           speed_deviation)
speed_level_fast = fuzz.interp_membership(x_speed, speed_fast, speed_deviation)

distance_level_close = fuzz.interp_membership(x_distance, distance_close,
                                              distance_to_car)
distance_level_right = fuzz.interp_membership(x_distance, distance_right,
                                              distance_to_car)
distance_level_far = fuzz.interp_membership(x_distance, distance_far,
                                            distance_to_car)

print("Speed membership: " + str(speed_level_slow) + " " +
      str(speed_level_right) + " " + str(speed_level_fast))
print("Distance membership: " + str(distance_level_close) + " " +
      str(distance_level_right) + " " + str(distance_level_far))
        ...
        np.fmax(
            ...
            np.fmax(
                ...
                np.fmax(
                    throttle5,
                    np.fmax(throttle6,
                            np.fmax(throttle7, np.fmax(throttle8,
                                                       throttle9))))))))
...
throttley = fuzz.interp_membership(x_throttle, aggregated,
                                   throttlesetting)  # for plot
...
ax0.plot(x_throttle, throttle_maintain, 'g', linewidth=0.5, linestyle='--')
ax0.plot(x_throttle, throttle_acc, 'r', linewidth=0.5, linestyle='--')
ax0.fill_between(x_throttle,
                 thottle0,
                 aggregated,
                 facecolor='Orange',
                 alpha=0.7)
ax0.plot([throttlesetting, throttlesetting], [0, throttley],
         'k',
         linewidth=1.5,
         alpha=0.9)
